...
import { knn, cosineSimilarity } from '../lib/knn';
...
export default function Comparator() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      <p className="text-text-muted mb-8">Compare two document populations by cross-neighborhood similarity.</p>

      <div className="flex items-end gap-4 mb-8">
        <div>
          <label className="block text-sm text-text-muted mb-1">Population A</label>
          <select
            value={popA}
        ...
        <div>
          <label className="block text-sm text-text-muted mb-1">Population B</label>
          <select
            value={popB}
      ...
      {results && (
        <div>
          ...
          <div className="bg-bg-surface border border-border-line rounded-lg p-6 mb-6">
            <h3 className="text-text-primary font-medium mb-4">Similarity Distribution</h3>
            <div className="flex items-end gap-1 h-32">
              {(() => {
                ...
                return bins.map((count, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-accent-cyan/60 rounded-t hover:bg-accent-cyan transition-colors"
                    style={{ height: `${(count / maxBin) * 100}%` }}
                    title={`${(i / 20).toFixed(2)}-${((i + 1) / 20).toFixed(2)}: ${count}`}
                  />
                ));
              })()}
            </div>
          ...
          <div className="bg-bg-surface border border-border-line rounded-lg overflow-hidden">
            ...
            {results.neighborRows.length > 100 && (
              <div className="px-4 py-2 text-xs text-text-muted border-t border-border-line">
                Showing 100 of {results.neighborRows.length} pairs
              </div>
            )}
          </div>
        </div>
